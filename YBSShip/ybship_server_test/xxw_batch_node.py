...
if __name__ == '__main__':
    ...
    exnode01_target_path = '/home/' + sshinfo_krace[
        'username'] + '/Documents/iov_BC_experiment/run_' + node_name + '.sh'
    target_folder_path = '/home/' + sshinfo_krace['username'] + '/Documents/iov_BC_experiment'
    plists.append(multiprocessing.Process(target=restart_geth, args=(
        sshinfo_krace, run_exnode01_path, exnode01_target_path, target_folder_path, node_name)))
    # restart_geth(sshinfo_krace, run_exnode01_path, exnode01_target_path, target_folder_path, node_name)
    plists[-1].start()
    plists[-1].join(5)
    plists[-1].terminate()
    # time.sleep(10)
    w3_list.append(Web3(Web3.HTTPProvider('http://' + sshinfo_krace['host'] + ':8545')))
    print('blocknumber: ', w3_list[-1].eth.blockNumber)

    rnode_list.append(remote_node_module.RNode(sshinfo_krace, 'exnode02',
                                               ('/home/' + sshinfo_krace['username'] + '/Documents/iov_BC_experiment'),
                                               './run_exnode02.sh',
                                               '/home/' + sshinfo_krace[
                                                   'username'] + '/Documents/iov_BC_experiment/run_' + 'exnode02' + '.sh'))

    plists.append(multiprocessing.Process(target=restart_geth, args=(rnode_list[-1].ssh_info,
                                                                     rnode_list[-1].sh_local_path,
                                                                     rnode_list[-1].sh_remote_path,
                                                                     rnode_list[-1].node_foder_path,
                                                                     rnode_list[-1].node_name)))

    plists[-1].start()
    plists[-1].join(5)
    plists[-1].terminate()
    # time.sleep(10)
    w3_list.append(Web3(Web3.HTTPProvider('http://' + sshinfo_krace['host'] + ':8546')))
    print('nodeInfo: ', w3_list[-1].geth.admin.nodeInfo())